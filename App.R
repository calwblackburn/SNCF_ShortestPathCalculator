...
## Import relevant data for stations, lines
...
stations_in_france = stations_in_france %>%
...
station_labels = paste(
  ...
  "<br><strong>Degree Centrality:</strong>", stations_in_france$degree,
  "<br><strong>Closeness Centrality:</strong>", round(stations_in_france$closeness, 4),
  "<br><strong>Betweenness Centrality:</strong>", round(stations_in_france$betweenness, 4)
)
...
server = function(input, output, session) {
  ...
  output$map = renderLeaflet({
    leaflet(stations_in_france) %>%
      ...
      addMarkers(icon = station_icon, lng = ~longitude, lat = ~latitude, popup = station_labels) %>%
      addPolylines(data = lines, color = ~color, label = ~class)
  })
